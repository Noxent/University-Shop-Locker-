...
## 🔌 1. ESP32 to Relay Module (Low Voltage Side)

The ESP32 sends 3.3V logic signals to the relay board to trigger the switches.

* **ESP32 5V (VIN) Pin** ➡️ 5V Output from Buck Converter
* **ESP32 GND Pin** ➡️ Ground from Buck Converter
* **Relay VCC / JD-VCC** ➡️ 5V Output from Buck Converter
* **Relay GND** ➡️ Ground from Buck Converter
...
## 🧲 2. Relay Module to Solenoids (High Voltage Side)
...
1. **12V Power Supply (+)** ➡️ Connects to the **COM (Common)** terminal on the Relay.
2. **Relay NO (Normally Open)** ➡️ Connects to the **Positive (+)** wire of the Solenoid.
3. **12V Power Supply GND (-)** ➡️ Connects directly to the **Negative (-)** wire of the Solenoid.
...
## 🛡️ 3. The Flyback Diode (CRITICAL SAFETY STEP)
